  ...
  useEffect(() => {
    const fetchTasks = async () => {
      try {
        ...
        // Don't navigate here - let the interceptor handle it
      } finally {
        setLoading(false);
      }
    };

    // Only fetch tasks if we have a token
  ...
  const handleUpdate = async (taskData: Omit<Task, "id" | "status">) => {
    ...
    try {
      const { data } = await api.put(`/tasks/${editingTask.id}`, {
        ...taskData,
        status: editingTask.status, // Maintain current status
      });
